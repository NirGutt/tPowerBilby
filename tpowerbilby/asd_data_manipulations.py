...
def create_peaks_prior_old(x,y,plotit=False):
    ...
    y_med = asd_utilies.med_rolling(y, 101)
...
def get_GW_data_old(config,start_time,f_i=None,f_f=None):
    ...
    #data = TimeSeries.get("H1:GWOSC-4KHZ_R1_STRAIN", psd_start_time, psd_end_time, host="losc-nds.ligo.org")
    psd_alpha = 2 * roll_off / psd_duration
    psd = psd_data.psd(
...
def load_data_bank(file_path):
    data_bank0={}
    if os.path.isfile(file_path):
        with open(file_path, 'rb') as file:
            print('Loading data bank...')
            data_bank0 = pickle.load(file)

    else:
        print('didnt load the data bank ')

    return data_bank0    

data_bank={}
data_bank=load_data_bank('data_bank.pkl') 


def get_GW_data(config,start_time,f_i=None,f_f=None):
    ...
    print('looking for data in out local database')
    print(key)
    if key in data_bank:
        print('found in data base ')
        return data_bank[key]
    else:
        print('not in data bank ')
